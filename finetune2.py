...
class MSELossLogger(tf.keras.callbacks.Callback):
    def __init__(self):
        super().__init__()
        self.epoch_losses = []
...
ckpt_dir = '/content/drive/MyDrive/stable_diffusion_4x4/diffusion_model_stage_8'
...
diffusion_ft_trainer.fit(
    training_dataset,
    epochs=epochs,
    callbacks=[custom_ckpt_callback, model_checkpoint_callback, mse_logger]
)

# Set directory for saving plot
plot_save_dir = "/content/drive/MyDrive/stable_diffusion_4x4/diffusion_model_stage_8/plots_mse"
os.makedirs(plot_save_dir, exist_ok=True)
plot_path = os.path.join(plot_save_dir, "mse_training_loss_150_epochs.png")

# Plot
plt.figure(figsize=(10, 5))
plt.plot(range(1, len(mse_logger.epoch_losses) + 1), mse_logger.epoch_losses, label="MSE Loss")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Training MSE Loss Curve (150 Epochs)")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
print(f"Saved MSE training loss plot to: {plot_path}")


loss_df = pd.DataFrame({"epoch": list(range(1, len(mse_logger.epoch_losses) + 1)),
                        "mse_loss": mse_logger.epoch_losses})
csv_path = os.path.join(plot_save_dir, "mse_training_loss_150_epochs.csv")
loss_df.to_csv(csv_path, index=False)
print(f"Saved MSE loss values to CSV: {csv_path}")