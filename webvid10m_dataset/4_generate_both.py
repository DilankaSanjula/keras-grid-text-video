...
def create_grid_image_4x4(frames, grid_size=(4, 4), output_path='grid_image.jpg'):
...
def create_grid_image_2x2(frames, grid_size=(2, 2), output_path='grid_image.jpg'):
    frame_height, frame_width = frames[0].shape[:2]
    grid_height = grid_size[0] * frame_height
    grid_width = grid_size[1] * frame_width
    ...
    indices = [0, 5, 10, 15]  # Indices of frames to be used for the 2x2 grid
    ...
    for filename in os.listdir(folder_path):
        try:
            if filename.endswith(('.mp4', '.avi', '.mov')):
                ...
                if frames:
                    output_path_4x4 = os.path.join(output_folder_4x4, f"Grid_image_of_{os.path.splitext(filename)[0]}.jpg")
                    output_path_2x2 = os.path.join(output_folder_2x2, f"Grid_image_of_{os.path.splitext(filename)[0]}.jpg")
                    create_grid_image_2x2(frames, grid_size=grid_size_2x2, output_path=output_path_2x2)
                    create_grid_image_4x4(frames, grid_size=grid_size_4x4, output_path=output_path_4x4)
        except:
            print("skipped")
